...
export enum ApiList {
  ...
  LIVE_EQUITY_DERIVATIVES = "/api/liveEquity-derivatives?index=top20_contracts",
}
...
export class NseIndia {
  ...
  async getLiveEquityDerivatives(): Promise<string[]> {
    const { data } = await this.getDataByEndpoint(
      ApiList.LIVE_EQUITY_DERIVATIVES
    );
  ...
  async getAllStockSymbols(): Promise<string[]> {
    ...
    return data
      .map((obj: { metadata: { symbol: string } }) => obj.metadata.symbol)
      .sort();
  }
  /**
   *
  ...
  getOptionChain(symbol: string): Promise<OptionChainData> {
    return this.getDataByEndpoint(
      `${ApiList.NSE_OPTION_CHAIN}${encodeURIComponent(symbol.toUpperCase())}`
    );
  }
